    ...
    async execute(interaction) {
        ...
        switch (choices) {
            case "roles":
                {
                    const master =
                        interaction.guild.roles.cache.find((r) => r.name == "Master") ||
                        null;
                    const diamond =
                        interaction.guild.roles.cache.find((r) => r.name == "Diamond") ||
                        null;
                    const platinum =
                        interaction.guild.roles.cache.find((r) => r.name == "Platinum") ||
                        null;
                    const gold =
                        interaction.guild.roles.cache.find((r) => r.name == "Gold") || null;
                    const silver =
                        interaction.guild.roles.cache.find((r) => r.name == "Silver") ||
                        null;
                    const bronze =
                        interaction.guild.roles.cache.find((r) => r.name == "Bronze") ||
                        null;
                                ...
                                new SelectMenuBuilder()
                                    ...
                                    .setOptions(
                                        new SelectMenuOptionBuilder({
                                            label: "📣 Новости",
                                            value: ROLE_NEWS_NOTIFY,
                                        }),
                                        new SelectMenuOptionBuilder({
                                            label: "🎬 Стримы",
                                            value: ROLE_STREAM_NOTIFY,
                                        }),
                                        new SelectMenuOptionBuilder({
                                            label: "🔎 Поиск игроков",
                                            value: ROLE_FIND_NOTIFY,
                                        })
                                    )
                            ),
                        ],
                    ...
                    interaction.channel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setColor("#00FF00")
                                .setTitle(
                                    `Добро пожаловать в сообщество ${interaction.guild.name}`
                                )
                                .setDescription(
            ...
            case "rules":
                {
                    ...
                    if (!channel)
                        return interaction.editReply({
                            content: `В сообществе нет канала правил.`,
                        });
                    ...
                    await channel.send({
                        embeds: [
                            new EmbedBuilder()
                                .setColor(`#2f3136`)
                                .setTitle(
                                    `Будьте дружелюбны, вежливы и с уважением относитесь к другим участникам сообщества!`
                                )
                                .setDescription(
                                ...
                                .setFooter({
                                    text: `Администрация  вправе использовать свое суждение, когда будет иметь дело с деструктивным или иным неподобающим поведением.`,
                                }),
                        ],
                        components: [
            ...
            case "info":
                {
                    const { PREMIUM_ROLE_ID, COMMANDS_CHANNEL_ID } = require("../../config.json");
                    await interaction.channel.send({
                        content:
                            ...
                            + `Что бы дать доступ к своему каналу вы можете использовать команды в канале <#${COMMANDS_CHANNEL_ID}> или кнопки ниже.\n`
                            + `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`,
                        ...
                        components: [
                            new ActionRowBuilder().addComponents(
                                ...
                                new ButtonBuilder()
                                    .setCustomId("private_room_addFriends_button")
                                    .setLabel("Дать доступ")
                                    .setStyle(ButtonStyle.Success),

                                new ButtonBuilder()
                                    .setCustomId("private_room_removeFriends_button")
                ...
                const message = await interaction.channel.send({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setTitle(`Taego`)
                            .setURL(
                                `https://cdn.discordapp.com/attachments/973994370401448007/999308949318795324/nfuj_d.png`
                            )
                            .setImage(
                                `https://cdn.discordapp.com/attachments/973994370401448007/999308949318795324/nfuj_d.png`
                            ),
                ...
                const message2 = await interaction.channel.send({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .setTitle(`Deston`)
                            .setURL(
                                `https://cdn.discordapp.com/attachments/973994370401448007/999303451941011576/Deston-goTop.png`
                            )
                            .setImage(
                                `https://cdn.discordapp.com/attachments/973994370401448007/999303451941011576/Deston-goTop.png`
                            ),
            ...
            //                 .setDescription(`**Нажмите на кнопку для создания нужного Вам билета**`)
            //                 .setImage('https://cdn.discordapp.com/attachments/981126311126925322/1005422014527451196/aEdUU60CCnU.jpg')
            //         ],
            //         components: [
            //             new ActionRowBuilder().addComponents(

            //                 new ButtonBuilder()
            //                     .setCustomId('support_admin')
            //                     .setStyle(ButtonStyle.Primary)
            //                     .setLabel('Связаться с администрацией'),

            //                 new ButtonBuilder()
            //                     .setCustomId('support_task')
            //                     .setStyle(ButtonStyle.Success)
            //                     .setLabel('Предложение'),
